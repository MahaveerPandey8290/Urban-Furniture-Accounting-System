import { SalesOrderRepository } from './sales-orders.repository.js';
...
export class SalesOrderService {
  static async createOrder(dto: any, companyId: number, userId: number, requestId: string) {
    return prisma.$transaction(async (tx) => {
      const order = await SalesOrderRepository.create(companyId, dto, userId, tx);
  ...
  static async listOrders(query: any, companyId: number, contactId?: number) {
    ...
    return SalesOrderRepository.findAll(companyId, effectiveQuery);
  }
  ...
  static async getOrder(id: number, companyId: number, contactId?: number) {
    ...
    if (!order) throw new NotFoundError('SalesOrder not found');
    if (contactId && order.customerId !== contactId) throw new ForbiddenError('Access denied');
    return order;
  }

  static async confirmOrder(id: number, companyId: number, userId: number, requestId: string) {
    return prisma.$transaction(async (tx) => {
      const order = await SalesOrderRepository.findById(id, companyId, tx);
      if (!order) throw new NotFoundError('SalesOrder not found');
      if (order.status !== 'DRAFT') throw new ConflictError('Only DRAFT orders can be confirmed');

      const updated = await SalesOrderRepository.updateStatus(id, companyId, 'CONFIRMED', tx);
      await AuditService.log({
        companyId,
        userId,
        entity: 'SalesOrder',
  ...
  static async cancelOrder(id: number, companyId: number, userId: number, requestId: string) {
    return prisma.$transaction(async (tx) => {
      ...
      const updated = await SalesOrderRepository.updateStatus(id, companyId, 'CANCELLED', tx);
      await AuditService.log({
        companyId,
        userId,
        entity: 'SalesOrder',
